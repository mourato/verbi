...
RAW_ALPHA="$("$MAGICK" identify -format '%A' "$RAW_ICON")"
if [[ "$RAW_ALPHA" == "Blend" || "$RAW_ALPHA" == "True" ]]; then
...
read -r MASTER_WIDTH MASTER_HEIGHT < <("$MAGICK" identify -format '%w %h\n' "$MASTER_FILE")
[[ "$MASTER_WIDTH" == "$CANVAS_SIZE" && "$MASTER_HEIGHT" == "$CANVAS_SIZE" ]] \
  || die "unexpected ${CANVAS_SIZE}px master size: got ${MASTER_WIDTH}x${MASTER_HEIGHT}"

MASTER_ALPHA="$("$MAGICK" identify -format '%A' "$MASTER_FILE")"
if [[ "$FULL_BLEED" -eq 1 ]]; then
  if [[ "$MASTER_ALPHA" == "Blend" || "$MASTER_ALPHA" == "True" ]]; then
    ...
    # Opaque full-bleed masters (no alpha channel) already fill the canvas.
    ACTUAL_TRIM="${CANVAS_SIZE}x${CANVAS_SIZE}+0+0"
  fi
  echo "Generated full-bleed AppIcon assets: $APPICONSET_DIR"
else
  [[ "$MASTER_ALPHA" == "Blend" || "$MASTER_ALPHA" == "True" ]] \
    || die "padded AppIcon masters require an alpha channel"
