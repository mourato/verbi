...
For repos of any real size, fan out with parallel read-only `explorer` agents — one per category (or cluster of related categories). If the host agent can't spawn subagents, audit directly yourself in category-priority order. **Subagents do not inherit this skill's task context**, so each child prompt must include:
