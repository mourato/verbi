...
Spawn **one** global `implementer` custom agent with worktree isolation. The parent task chooses the model from the global agent catalog; do not hardcode provider-specific model names in this skill.
